...
func GetPetsInfoByUserId(id int64) (pets []*models.PetInfo, err error) {
	pets, err = mysql.GetPetsInfoByUserId(id)
	if err != nil {
		zap.L().Error("mysql.GetPetsInfoByUserId(id) failed",
			zap.Int64("id", id),
			zap.Error(err))
	}
	return
...
func GetClerkList() (clerks []*models.ClerkList, err error) {
	clerks, err = mysql.GetClerkList()
	if err != nil {
		zap.L().Error("mysql.GetClerkList() failed",
			zap.Error(err))
	}
	return
}

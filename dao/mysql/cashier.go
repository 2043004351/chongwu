...
func GetPetsInfoByUserId(id int64) (pets []*models.PetInfo, err error) {
	sqlStr := `select pet_id,pet_name from pet where owner_id=?`

	err = db.Select(&pets, sqlStr, id)
	return
}

func GetClerkList() (pets []*models.ClerkList, err error) {
	sqlStr := `select clerk_id,clerk_name from clerk`

	err = db.Select(&pets, sqlStr)
	return
}
